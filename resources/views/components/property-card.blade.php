@php
	...
	if (isset($property)) {
	    ...
	    $property['type'] = $property->type ? $property->type : 'N/A';
	} else {
	    $property = (object) [
	        'status' => 'For Rent',
...
<div class="h-full container relative overflow-hidden rounded-xl bg-transparent shadow-xl">
	...
	<div class="h-full flex flex-col gap-2 p-6">
		<div class="flex items-end justify-between gap-2 py-2">
			...
			<x-button class="btn-sm rounded bg-primary transition-all ease-in active:scale-95 text-white">View Units</x-button>
		</div>
